...
def get_valid_triplets_mask(labels):
    ...
    indices_equal = torch.zeros_like(labels.expand(labels.size(0),labels.size(0))).byte()
    torch.nn.init.eye_(indices_equal)
...
class TripletLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=False,batch_hard='yes'):
        ...
        if batch_hard=='yes':
            ...
            dist_ap = dist_mat.unsqueeze(2)
            dist_an = dist_mat.unsqueeze(1)
...
class MarginSampleMiningLoss(object):
    """Modified from Tong Xiao's open-reid (https://github.com/Cysu/open-reid).
    Related Triplet Loss theory can be found in paper 'In Defense of the Triplet
    Loss for Person Re-Identification'."""
